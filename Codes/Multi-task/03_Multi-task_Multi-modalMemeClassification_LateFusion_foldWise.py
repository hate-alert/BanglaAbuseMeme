...
class Combined_model(nn.Module):
    def __init__(self, text_model, image_model):
        ...
        self.abusive   = nn.Linear(2*64,2)
        self.sentiment = nn.Linear(2*64,3)
        self.sarcasm   = nn.Linear(2*64,2)
        self.vulgar    = nn.Linear(2*64,2)
...
outputFp = open("MultiTaskResultFolder/MultiTask_MMFoldWiseLateFusion_LessParam.txt", 'a')
...
for text_mod in text_models:
    for image_mod in image_models:
        modelName = "new_lt_"+text_mod+"_"+image_mod
        ...
        for fold in allF:
            ...
            comb = Combined_model(tex, img).to(device)
