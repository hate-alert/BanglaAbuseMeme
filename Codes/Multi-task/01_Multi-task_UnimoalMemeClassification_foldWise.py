...
def getFeaturesandLabel(X, abusive_labels, sentiment_labels, sarcasm_labels, vulgar_labels, text_features):
    X_text_data = []
    for i in X:
    ...
    return X_text_data, y_abusive, y_sentiment, y_sarcasm, y_vulgar
...
def getPerformanceOfLoader(model,test_dataloader, loadType):
    ...
    # Add batch to GPU
        ...
        b_text_ids, y_abusive, y_sentiment, y_sarcasm, y_vulgar = batch
...
def trainModel(model, train_dataloader, validation_dataloader, test_dataloader):
    ...
    for epoch_i in range(0, epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            b_abusive   = batch[1].to(device)
            b_sentiment = batch[2].to(device)
            b_sarcasm   = batch[3].to(device)
            b_vulgar    = batch[4].to(device)
            model.zero_grad()        
...
outputFp = open("MultiTaskResultFolder/MultiTask_unimodalFoldWiseLessParam.txt", 'w')


for modelName in modelNameMapping:

    with open(modelNameMapping[modelName],'rb') as fp:
        inputDataFeatures = pickle.load(fp)

    # Audio parameters
    input_size_audio = len(inputDataFeatures['image_759.jpg'])
    ...
    for fold in allF:
        ...
        X_train_text, y_train_abs, y_train_senti, y_train_sar, y_train_vul = getFeaturesandLabel(train_list, train_abusive, train_sentiment, train_sarcasm, train_vulgar, inputDataFeatures)
        X_val_text, y_val_abs, y_val_senti, y_val_sar, y_val_vul = getFeaturesandLabel(val_list, val_abusive, val_sentiment, val_sarcasm, val_vulgar, inputDataFeatures)
        X_test_text, y_test_abs, y_test_senti, y_test_sar, y_test_vul = getFeaturesandLabel(test_list, test_abusive, test_sentiment, test_sarcasm, test_vulgar, inputDataFeatures)


        BATCH_SIZE = 32
        #Dataset wrapping tensors.
        train_data = TensorDataset(X_train_text, y_train_abs, y_train_senti, y_train_sar, y_train_vul)
        val_data = TensorDataset(X_val_text, y_val_abs, y_val_senti, y_val_sar, y_val_vul)
        test_data = TensorDataset(X_test_text, y_test_abs, y_test_senti, y_test_sar, y_test_vul)
        ...
        comb = Uni_Model(input_size_audio, fc1_hidden_audio, fc2_hidden_audio).to(device)
    ...
    with open("MultiTaskResultFolder/MultiTask_"+modelName+"_foldWise_res_LessParam.p", 'wb') as fp:
