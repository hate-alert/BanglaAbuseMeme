...
for modelName in modelNameMapping:
    ...
    for fold in allF:
        ...
        X_train_text, y_train = getFeaturesandLabel(train_list, train_label, inputDataFeatures)
        X_val_text, y_val = getFeaturesandLabel(val_list, val_label, inputDataFeatures)
        X_test_text, y_test = getFeaturesandLabel(test_list, test_label, inputDataFeatures)

        
        BATCH_SIZE = 32
        #Dataset wrapping tensors.
        train_data = TensorDataset(X_train_text, y_train)
        val_data = TensorDataset(X_val_text, y_val)
        test_data = TensorDataset(X_test_text, y_test)
        ...
        comb = Uni_Model(input_size_audio, fc1_hidden_audio, fc2_hidden_audio,  2).to(device)
    ...
    with open("ResultFolder/"+modelName+"_foldWise_res_LessParam.p", 'wb') as fp:
